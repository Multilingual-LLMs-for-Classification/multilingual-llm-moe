...
class PPOPolicy(nn.Module):
    def __init__(self, state_dim, num_llms, hidden=256):
        super().__init__()
...
class PPOAgent:
    ...
    def select_action(self, state):
        ...
        action = dist.sample()
        return action.item(), dist.log_prob(action), value.detach()  # ✅ detach value
    ...
    def compute_returns(self, rewards, values, dones):
        ...
        values = torch.cat(values).squeeze()  # already detached
        advantages = returns - values
        return returns, advantages.detach()  # ✅ detach advantages
...
class RoutingPipeline:
    def __init__(self, agent, llm_system, state_dim):
        ...
        self.encoder = MBertStateEncoder()
    ...
    def train_step(self, batch, output_csv=None):
        ...
        for _, row in batch.iterrows():
            ...
            pred_rating, decoded, prompt, model_name = self.llm_system.run(
            ...
            if output_csv:
                with open(output_csv, "a", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        row['review_body'], row['language'], row['stars'],
                        action, pred_rating, decoded, prompt, model_name
                    ])

            states.append(state)
            actions.append(action)
            old_log_probs.append(log_prob.detach())  # ✅ detach
...
if __name__ == "__main__":
    ...
    agent = PPOAgent(state_dim, len(model_names))
    ...
    for r in range(ROUNDS):
        ...
        torch.save(agent.policy.state_dict(), f"ppo_router_mbert_round{r+1}.pth")
