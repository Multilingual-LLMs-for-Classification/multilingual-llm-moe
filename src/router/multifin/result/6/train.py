...
class PPOAgent:
    def __init__(self, state_dim, num_llms, lr=4e-4, gamma=0.99, clip_eps=0.1, entropy_weight=0.01):
        ...
        # Track how often each LLM is selected globally
        self.global_usage = np.zeros(num_llms)

    def select_action(self, state, temperature=2.0):
        ...
        action = dist.sample()
        action_idx = min(action.item(), probs.shape[-1]-1)
        self.global_usage[action_idx] += 1
        return action_idx, dist.log_prob(action), value
...
class RoutingPipeline:
    ...
    def reward_fn(self, model_id, pred_label, true_label, lang):
        ...
        # Exploration bonus: higher if model/lang combo is rarely used
        usage_count = len(self.lang_history[model_id][lang])
        exploration_bonus = 1.0 / (usage_count + 1)

        # Global usage bonus (optional)
        global_usage_penalty = 1.0 / (self.agent.global_usage[model_id] + 1)

        final_reward = 0.35 * per_sample + 0.35 * lang_accuracy + 0.15 * exploration_bonus + 0.15 * global_usage_penalty
