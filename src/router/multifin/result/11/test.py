...
STATE_DIM = 768 + NUM_LANGS  # XLM-R embeddings have 768 dims
...
class XLMREmbedder:
    ...
    def encode(self, texts):
        ...
        return pooled.cpu().numpy() if len(pooled) == 1 else pooled.cpu().numpy()
...
def run_test():
    ...
    router.load_state_dict(torch.load("neural_bandit_buffer_embedding_ucb2.pth"))
    router.eval()

    output_csv = "test_detailed_results_buffer_xlmr2.csv"
    ...
    for _, row in tqdm(test_df.iterrows(), total=len(test_df)):
        # Feature Engineering
        emb = embedder.encode(row['text'])[0]  # XLM-R embedding
        ...
        with torch.no_grad():
            q_vals = router(context.unsqueeze(0))
    ...
    metrics_df.to_csv("language_wise_metrics_xlmr2.csv", index=False)
